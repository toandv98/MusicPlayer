...
import com.toandv.demo.musicplayer.data.SongProvider
...
class MainActivity : AppCompatActivity() {
    ...
    private var listSongs = arrayListOf<Song>()
    ...
    private val playerServiceConnection = object : ServiceConnection {
        ...
        override fun onServiceConnected(componentName: ComponentName?, iBinder: IBinder?) {
            ...
            playerService.updatePlayList(listSongs)
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            when (intent?.action) {
                Actions.ACTION_PREPARED -> {
                    updateSongDetail(intent.getParcelableExtra(Extras.EXTRA_SONG))
                    intent.getIntExtra(Extras.EXTRA_DURATION, 0).let {
                        seekDuration.max = it
                        tvDuration.text = TimeUtils.toTimer(it)
                    }
                }
                ...
                Actions.ACTION_SEEK_CHANGED -> {
                    intent.getIntExtra(Extras.EXTRA_POSITION, 0).let {
                        ...
                        tvCurrentTime.text = TimeUtils.toTimer(it)
                    }
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listSongs.run {
            clear()
            addAll(SongProvider.getAllDeviceSongs(this@MainActivity))
            songAdapter.submitList(this)
            if (listSongs.isNotEmpty()) updateSongDetail(first())
        }
    ...
    override fun onResume() {
        ...
        if (isBound) playerService.restoreDetail()
    }
    ...
    private fun updateSongDetail(song: Song?) {
        song?.run {
